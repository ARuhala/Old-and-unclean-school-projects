...
void suorita_suorita(Todo& tehtavat, const vector<string>& kentat) {
    if ( kentat.size() != 1 ) {
        virheilmoitus("listaa: liian paljon sanoja syoterivilla.");

    } else {
        // funktion vastaanottama tehtavat on muotoa
        // map<int, list<string>>; joka on olion tietoihin talletettuna muuttujaan Todo

        // tarkistetaan ettei mappi ole tyhjä
        if ( not tehtavat.mapinkoontarkastus() ) {
            virheilmoitus("suorita: tehtavia ei ole jaljella.");
            return;
        }

        tehtavat.suorita_tehtava();
    }
}
