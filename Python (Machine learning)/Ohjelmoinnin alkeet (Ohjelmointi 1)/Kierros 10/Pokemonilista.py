...
def main():
    ...
    while syöte != "":
        osat = syöte.split(",")
        laji = osat[0]
        #luo tyypeistä listan
        if len(osat) > 2:
            tyypit = osat[1].split(" ")
        else:
            tyypit = osat[1].strip()
        osat[0] = Pokemon("{}".format(laji.capitalize()), tyypit)
        lista.append(osat[0])
        syöte = input()

    numero=1
    for pokemoni in lista:

        print("{}. syöttämäsi Pokemon on".format(numero),end="")
        pokemoni.tulosta()
        numero = numero + 1
